<script>
    ...
    async function getdata(endpoint){
        const res = await fetch(`/all-files${(endpoint ? '/'+endpoint:'')}`, { method: "POST" })
    ...
    let query = ""
    let searchFile = async (e) => {
        e.target.querySelector("button").innerText = "Loading..."
        filedata = await getdata(`${query == "" ? "":query}`)
...
{/if}
<div class="search">
    <form on:submit|preventDefault="{e => searchFile(e)}">
